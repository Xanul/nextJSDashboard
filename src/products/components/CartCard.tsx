...
import { addProductCookieCart, removeSingleItemCookieCart } from "@/shopping-cart/actions/actions";
...
export const CartCard = ({ product, quantity }: Props) => {
  const router = useRouter();

  function onAddToCart() {
    addProductCookieCart(product.id)
    router.refresh();
  }

  const onRemoveItem = () => {

    removeSingleItemCookieCart(product.id);
    router.refresh();
    ...
    <div className="flex items-center shadow rounded-lg w-full bg-white border-gray-100">
      ...
      <div className="px-5 pb-5 w-full flex flex-col mt-2">
        ...
        <div className="flex flex-col items-start justify-between">
          <span className=" dark:text-white">Cantidad: {quantity}</span>
